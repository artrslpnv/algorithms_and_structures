...
public:
    explicit SuffixArrayFounder(const std::string &s) : size(s.size()), suffix_arrays(s.size()),
        ...
        long long amount_of_classes = FirstStepCountingSort(counting, s);
...
private:
    ...
    long long FirstStepCountingSort(std::vector<long long> &counting, const std::string &s) {
        for (int i = 0; i < s.size(); ++i) {
            ++counting[s[i]];
        }
...
long long CountDifferentSubstr(std::string &s) {
    ...
    long long ans = 0;
    for (int i = 1; i < S.show_size(); ++i) { // c 1 потому что игнорируем 0 элемент
